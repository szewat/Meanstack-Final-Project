...
galleryApp.controller("pageControllers", function($scope, FIREBASE_URL, $firebaseArray, $firebaseObject){
    ...
    $scope.addPortfolio = function(new_portfolio){
        // console.log("dd made it");

            if (document.getElementById("mystilllife").checked == true){

                var pushStillRef = new Firebase(FIREBASE_URL + '/stilllifes');
                pushStillRef.push(
                {
                    username: new_portfolio.username,
            ...
            else if (document.getElementById("mylandscape").checked == true){
                
                var pushLandRef = new Firebase(FIREBASE_URL + '/landscapes');
                pushLandRef.push(
                {
                    username: new_portfolio.username,
                    creationDate: Firebase.ServerValue.TIMESTAMP,
...
// For the landscape section
    ...
    // not needed, this part of the code from classnotes previously broke the page
    // $scope.addPortfolio = function( new_portfolio ) {
